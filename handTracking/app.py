...
def logging_csv(number, mode, landmark_list, point_history_list):
    ...
    if mode == 1 and (0 <= number <= 9):
        csv_path = 'handTracking/model/keypoint_classifier/keypoint.csv'
        with open(csv_path, 'a', newline="") as f:
            writer = csv.writer(f)
            writer.writerow([number, *landmark_list])
    if mode == 2 and (0 <= number <= 9):
        csv_path = 'handTracking/model/point_history_classifier/point_history.csv'
        with open(csv_path, 'a', newline="") as f:
            writer = csv.writer(f)
...
def get_bezier_points(image, brect, handedness, hand_sign_text,
    ...
    centrales.append(point1)
    centrales.append(point2)
    centrales.append(point3)
    centrales.append(point4)
    centrales = np.unique(centrales, axis=0)
    #print(centrales)


    if "Pointer" in hand_sign_text:
        np.save('centrales.npy', centrales)
        return True
    #else:
        #point1 = (0,0)
        #point2 = (0,0)
